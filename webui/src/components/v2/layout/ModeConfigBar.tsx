...
function ToggleButton<T extends string>({
  ...
  const inactive = options.find((o) => o.value !== value) ?? options[1];
  const isDefault = value === options[0].value;

  return (
    <div
      data-testid={testId}
      className="group inline-flex items-center rounded-v2-input bg-[var(--v2-input-bg)] border border-v2-border overflow-hidden"
    >
      {/* Active value — always visible */}
      <span
        className={cn(
          'px-3 py-1.5 text-xs font-semibold transition-colors duration-100',
          isDefault
            ? 'text-v2-text'
      ...
      {/* Inactive value — appears on hover */}
      <button
        type="button"
        onClick={() => onChange(inactive.value)}
        className={cn(
          'py-1.5 text-xs font-medium transition-all duration-150 whitespace-nowrap',
          'max-w-0 px-0 opacity-0 overflow-hidden',
          'group-hover:max-w-[80px] group-hover:px-3 group-hover:opacity-100',
    ...
    <div ref={ref} className="relative" data-testid={testId}>
      ...
      {open && (
        <div
          ...
          className={cn(
            'absolute bottom-full mb-1.5 left-0 z-60',
...
function AgentCountStepper({ onIncrement }: { onIncrement?: () => void }) {
  ...
  return (
    <div
      ...
      className="inline-flex items-center rounded-v2-input bg-[var(--v2-input-bg)] border border-v2-border overflow-hidden"
    >
      <button
        type="button"
...
function ModelChip({
  ...
  return (
    <span
      ...
      className={cn(
        'inline-flex items-center px-2 py-1 rounded text-[11px] font-medium transition-all',
...
function AgentSummaryButton({ onClick }: { onClick: () => void }) {
  ...
  if (!useConfigAgents && agentCount === null) {
    return (
      <button
        ...
        onClick={onClick}
        className={cn(
          'inline-flex items-center gap-1.5 px-2 py-1.5 rounded-v2-input',
  ...
  return (
    <div
      ...
      onClick={onClick}
      className={cn(
        'inline-flex items-center gap-1.5 px-1 py-1 pr-2 rounded-v2-input cursor-pointer',
        'text-xs border border-v2-border bg-[var(--v2-input-bg)]',
...
function AgentCard({ index }: { index: number }) {
  ...
  return (
    <div
      ...
      className={cn(
        'w-[280px] flex-shrink-0 rounded-lg border border-v2-border bg-v2-main',
        'p-3.5 flex flex-col gap-2.5 transition-colors',
        'hover:border-v2-accent/40'
      )}
    >
      {/* Header */}
      <div className="flex items-center justify-between">
        <span className="text-[13px] font-semibold flex items-center gap-2">
          <span className="w-6 h-6 rounded-full inline-flex items-center justify-center text-[11px] font-bold bg-v2-accent/15 text-v2-accent">
            {letter}
          </span>
          Agent {letter}
        </span>
...
function MaxRoundsSelector() {
  ...
  return (
    <div
      ...
      className="inline-flex items-center gap-1 px-2 py-1.5 rounded-v2-input bg-[var(--v2-input-bg)] border border-v2-border text-xs"
    >
      <span className="text-[10px] font-semibold uppercase tracking-wider text-v2-text-muted">
        Rounds
      </span>
    ...
    <>
      ...
      >
        ...
        <div className={cn(
          ...
          {/* Personas dropdown */}
          <DropdownToken<PersonasMode>
            items={PERSONAS_ITEMS}
            value={personasMode}
            defaultValue="off"
            onChange={setPersonasMode}
            testId="dropdown-personas"
          />
        ...
        <button
          ...
          className={cn(
            'inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-v2-input',
            'text-xs font-medium border transition-colors duration-100 ml-auto',
            dockerAvailable === false
        ...
        <button
          ...
          className={cn(
            'inline-flex items-center gap-1 px-2 py-1.5 rounded-v2-input',
